...
//    private ArrayList<String> loadSelected = new ArrayList<>();
    ...
    public String[] finalComment(DbOpenHelper mdb, ArrayList<String> loadSelected,
        ...
        String change = getTypeToChange(tempState, matState, thickState);
        if(tempComment>0){
            commentList[0] += " 현재 옷차림으로는 더울 수 있습니다."
                    + change;

        }
        else if(tempComment<0){
            commentList[0] += " 현재 옷차림으로는 추울 수 있습니다."
                    + change;
        }
        else{
            //덥다 춥다가 상쇄외어서 적절한 옷차림으로...?
            commentList[0] += " 기온에 적절한 옷차림입니다!";
        }
    ...
    private String getTypeToChange(int[] tempState, int[] matState, int[] thickState) {
        String result="";
        int index = -1;
